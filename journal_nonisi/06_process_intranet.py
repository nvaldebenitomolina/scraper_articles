...
# Input-Output
df_intranet = pd.read_csv("input_table/tracker_36.csv", sep=',',encoding='utf-8')
...
df = pd.DataFrame( columns = ["Item ID [*itemId:id]","Status [status:system]","Usuario [f_675:username]",
	"Reconocimiento o Afiliacion [f_687:code]","Estado [f_677:code]","Título [f_678:default]","Autores [f_679:default]",
	"Lineas Involucradas [f_681:code]","DOI [f_682:default]","Código ISSN [f_683:default]","Nombre Journal [f_684:default]",
	"Indexación por [indexacionPor:code]","Ubicación en Línea [f_685:default]","Año de Publicación [f_686:default]",
	"PARTICIPACION [f_689:code]","% aportado por REGIONAL [publicacionesjournalNoIsiA_rREGIONAL:default]",
	"% aportado por FONDECYT [f_692:default]","% aportado por FODEF [f_693:default]",
	"% aportado por PIA [publicacionesjournalNoIsiA_adoPorPIA:default]","% aportado por FONDAP [f_691:default]",
	"% aportado por PAI [publicacionesjournalNoIsiA_adoPorPAI:default]","% aportado por EXPLORA [publicacionesjournalNoIsiA_orEXPLORA:default]",
	"% aportado por colaboración Internacional [publicacionesjournalNoIsiA_rnacional:default]","% aportado por Astronomia [publicacionesjournalNoIsiA_stronomia:default]",
	"% aportado por Becas (PCHA) [publicacionesjournalNoIsiA_BecasPCHA:default]","% aportado por Otro (indicar) [f_696:default]",
	"Adjunta tu Publicación  [f_699:default]","Abstract","Revisión INTERNA [f_674:code]","Periodo [f_688:code]",
	"Líneas antiguas [f_680:code]"
])

#Add columns on new file Intranet
df["Item ID [*itemId:id]"] 		= ""
df["Status [status:system]"]	= ""
df["Título [f_678:default]"]	= title
df["Autores [f_679:default]"]	= authors_zotero
df["Lineas Involucradas [f_681:code]"]= line
df["DOI [f_682:default]"]			= doi_zotero
df["Código ISSN [f_683:default]"]	= issn
df["Nombre Journal [f_684:default]"]= journal
df["Indexación por [indexacionPor:code]"]	= ""
df["Ubicación en Línea [f_685:default]"]	= url
df["Año de Publicación [f_686:default]"]	= year
df["PARTICIPACION [f_689:code]"]	= ""
df["Reconocimiento o Afiliacion [f_687:code]"] = "SI"
df["Estado [f_677:code]"]		= "Publicadas"
df["% aportado por REGIONAL [publicacionesjournalNoIsiA_rREGIONAL:default]"] = "A COMPLETAR"
df["% aportado por FONDECYT [f_692:default]"]	= "A COMPLETAR"
df["% aportado por FODEF [f_693:default]"]		= "A COMPLETAR"
df["% aportado por PIA [publicacionesjournalNoIsiA_adoPorPIA:default]"] 	= "A COMPLETAR"
df["% aportado por FONDAP [f_691:default]"]	= "A COMPLETAR"
df["% aportado por PAI [publicacionesjournalNoIsiA_adoPorPAI:default]"] 	= "A COMPLETAR"
df["% aportado por EXPLORA [publicacionesjournalNoIsiA_orEXPLORA:default]"]	= "A COMPLETAR"
df["% aportado por colaboración Internacional [publicacionesjournalNoIsiA_rnacional:default]"] = "A COMPLETAR"
df["% aportado por Astronomia [publicacionesjournalNoIsiA_stronomia:default]"] 	= "A COMPLETAR"
df["% aportado por Becas (PCHA) [publicacionesjournalNoIsiA_BecasPCHA:default]"]= "A COMPLETAR"
df["% aportado por Otro (indicar) [f_696:default]"]	= "A COMPLETAR"
df["Adjunta tu Publicación  [f_699:default]"]		= ""
df["Abstract"]	= ""
df["Revisión INTERNA [f_674:code]"]	= "en zotero"
df["Periodo [f_688:code]"]			= "A REPORTAR 2020"
df["Líneas antiguas [f_680:code]"]	= ""

#Process like replace lines
df["Lineas Involucradas [f_681:code]"]=[str(d).replace('Ciudades Resilientes',':Cities') for d in df["Lineas Involucradas [f_681:code]"]]
df["Lineas Involucradas [f_681:code]"]=[d.replace('Cambio de Uso de Suelo',':Land Use Change') for d in df["Lineas Involucradas [f_681:code]"]]
df["Lineas Involucradas [f_681:code]"]=[d.replace('Zonas Costeras',':Coastal Zones') for d in df["Lineas Involucradas [f_681:code]"]]
df["Lineas Involucradas [f_681:code]"]=[d.replace('Agua y Extremos',':Water extremes') for d in df["Lineas Involucradas [f_681:code]"]]
df["Lineas Involucradas [f_681:code]"]=[str(d).replace('Gobernanza e Interfaz Ciencia y Política',':Governance') for d in df["Lineas Involucradas [f_681:code]"]]
df["Lineas Involucradas [f_681:code]"]=[d.replace('Transversal',':Transversal') for d in df["Lineas Involucradas [f_681:code]"]]
df["Lineas Involucradas [f_681:code]"]=[d.replace(';',',') for d in df["Lineas Involucradas [f_681:code]"]]
...
df["Usuario [f_675:username]"] = users_all
df["Usuario [f_675:username]"] = [str(d).replace("'","").replace('[','').replace(']','').replace("''","") for d in df["Usuario [f_675:username]"]]
#print(df["Usuraio -- 739"])
#Create a new dataframe
df_intranet_created = pd.DataFrame(df, columns = ["Item ID [*itemId:id]","Status [status:system]","Usuario [f_675:username]",
	"Reconocimiento o Afiliacion [f_687:code]","Estado [f_677:code]","Título [f_678:default]","Autores [f_679:default]",
	"Lineas Involucradas [f_681:code]","DOI [f_682:default]","Código ISSN [f_683:default]","Nombre Journal [f_684:default]",
	"Indexación por [indexacionPor:code]","Ubicación en Línea [f_685:default]","Año de Publicación [f_686:default]",
	"PARTICIPACION [f_689:code]","% aportado por REGIONAL [publicacionesjournalNoIsiA_rREGIONAL:default]",
...
print(result)
result.to_csv(str("output_table/tracker_36_modified.csv"), sep=',', encoding='utf-8', index=False)
